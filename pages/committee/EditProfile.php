...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        // Check if email is already taken by another committee member
        $stmt = $conn->prepare("SELECT id FROM committee WHERE email = ? AND id != ?");
        $stmt->bind_param("si", $email, $committeeId);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($result->num_rows > 0) {
            $error = 'Email is already taken by another committee member.';
            $stmt->close();
        } else {
            $stmt->close();
            ...
            if ($passwordChange) {
                ...
                if (empty($currentPassword)) {
                    ...
                    $stmt = $conn->prepare("SELECT password FROM committee WHERE id = ?");
                    ...
                    if (!password_verify($currentPassword, $committee['password'])) {
                        $error = 'Current password is incorrect.';
                    } else {
                        // Update committee with new password
                        $hashedPassword = password_hash($newPassword, PASSWORD_DEFAULT);
                        $stmt = $conn->prepare("UPDATE committee SET name = ?, email = ?, password = ? WHERE id = ?");
                        $stmt->bind_param("sssi", $name, $email, $hashedPassword, $committeeId);
                ...
                // Update committee without password change
                $stmt = $conn->prepare("UPDATE committee SET name = ?, email = ? WHERE id = ?");
                $stmt->bind_param("ssi", $name, $email, $committeeId);
...
$conn = getDBConnection();
$stmt = $conn->prepare("SELECT name, email FROM committee WHERE id = ?");
$stmt->bind_param("i", $committeeId);
$stmt->execute();
$result = $stmt->get_result();
$committee = $result->fetch_assoc();
...
<body>
    ...
    <div class="nav-bar">
        <div class="nav-bar-content">
            ...
            <div class="nav-links">
                <a href="CommitteeDashboard.php">Dashboard</a>
    ...
    <div class="profile-header">
        <div class="profile-content">
            <h1>Edit Profile</h1>
            <p>Update your committee profile information</p>
        </div>
    </div>
                ...
                <form method="post" action="" id="profileForm">
                    <div class="form-grid">
                        <div class="form-row">
                            ...
                            <input class="input" type="text" id="name" name="name" placeholder="Your full name" value="<?php echo htmlspecialchars($committee['name']); ?>" required />
                        </div>
                        <div class="form-row">
                            <label for="email">Email <span style="color: #dc2626;">*</span></label>
                            <input class="input" type="email" id="email" name="email" placeholder="committee@unikl.com" value="<?php echo htmlspecialchars($committee['email']); ?>" required />
                        </div>
                    </div>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="CommitteeDashboard.php" class="btn btn-outline">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
